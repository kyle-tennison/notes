...
Taking the time derivative of this gives us:

$$\mathbf{a}_A = \mathbf{a}_B + \boldsymbol{\alpha} \times \mathbf{r}_{A/B} + \boldsymbol{\omega} \times (\boldsymbol{\omega} \times \mathbf{r}_{A/B})$$
...
$$\mathbf v_A = \mathbf v_B + \boldsymbol \omega \times \mathbf r + \mathbf v_{A/B}$$


Where: 

$$\mathbf v_{A/B} = (\dot x \mathbf i + \dot y \mathbf j)$$

$${ \boldsymbol \omega  = (x \dot{\bf i} + y \dot{\bf j})}$$


See the [rotating axes diagram](#^rotating-axes-diagram) for clarification on each of these variables.
