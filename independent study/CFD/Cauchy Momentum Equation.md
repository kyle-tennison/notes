...
where $\textbf{u}$ is the velocity vector field, $t$ is time, $\frac{D \textbf{u}}{Dt}$ is the [Material Derivative](Material%20Derivatives.md) of the material element, $\rho$ is the density at the given point in the medium, $\boldsymbol{\sigma}$ is the [Stress Tensor](Stress%20Tensors.md), and $f = \begin{bmatrix}f_x\\f_y\\f_z\end{bmatrix}$.
...
$$\boldsymbol{\dot p}=(\nabla\cdot \boldsymbol{\sigma})dxdydz+\textbf{f}\rho dxdydz$$
...
$$\underbrace{\rho\left(\frac{\partial \textbf{u}}{\partial t}\right)+\rho (\textbf{u}\cdot \nabla)\textbf{u}=-\nabla p+\nabla \cdot \boldsymbol \tau + \rho \textbf a}_{\text{Momentum Equation}}$$
^momentum-equation
...
In the case that the fluid is incompressible, $\frac{D \rho}{D t} = 0$ and the equation reduces to $\nabla \cdot \textbf{u} =0$. 

---

Going back to the momentum equation:

$$\rho\left(\frac{\partial \textbf{u}}{\partial t}\right)+\rho (\textbf{u}\cdot \nabla)\textbf{u}=-\nabla p+\nabla \cdot \boldsymbol \tau + \rho \textbf a$$
...
> 🤖 (notecheck comment) - The above expression is conceptually incorrect. By applying the product rule correctly to $\frac{\partial (\rho \textbf{u})}{\partial t}$, it should be written as $\frac{\partial (\rho \textbf{u})}{\partial t} = \rho\,\frac{\partial \textbf{u}}{\partial t} + \textbf{u}\,\frac{\partial \rho}{\partial t}$, so that $\rho\,\frac{\partial \textbf{u}}{\partial t} = \frac{\partial (\rho \textbf{u})}{\partial t} - \textbf{u}\,\frac{\partial \rho}{\partial t}$. No term involving $\frac{\partial p}{\partial t}$ should appear here.
...
The acceleration is nonlinear and represented with the quantity $\textbf{u} \cdot \nabla \textbf{u}$.
