...
For two **continuous random variables** $X$ and $Y$, Bayes' Rule is largely unchanged:

$$f_{X|Y}(x|y) = \frac{f_{Y|X}(y|x)f_X(x)}{f_Y(y)}$$

Here, $f_X(x)$ is the pdf; $f_{Y|X}$ is a function of x and y, evaluated at a particular y (so not a pdf); and $f_Y(y)$ is a scalar. 

If you use the fact that:

$$f_Y(y) = \int \limits _{- \infty}^ \infty f_{Y|X}(y|u)f_X(u)\; du $$

You can write the rule as:

$$f_{X|Y}(x|y) = \frac{f_{Y|X}(y|x)f_X(x)}{\displaystyle \int \limits _{- \infty}^ \infty f_{Y|X}(y|u)f_X(u)\; du }$$

which is much easier to use in many scenarios. 

--- 

For **discrete random variables**, there is an analogous equation with a similar derivation:

$$p_{X|Y}(x|y) = \frac{p_{Y|X}(y|x)p_X(x)}{\displaystyle \sum _k p_{Y|X}(y|k)p_X(k)}$$

--- 

Sometimes learn that a *discrete* event $\{X=x\}$ occurs that changes the pdf for $Y$. In scenarios like this, we can mix pdfs and pmfs:

$$f_{Y|X}(y|x) = \frac{p_{X|Y}(x|y)f_Y(y)}{p_X(x)} = \frac{p_{X|Y}(x|y)f_Y(y)}{\displaystyle \int _{- \infty }^ \infty p_{X|Y}(x,u)f_Y(u)\;du}$$
...
Often, we want to go from a uniform distribution such as $X \sim \text{Uniform}([0,1])$ , to a non-uniform random variable $Z=g(X)$. The question is how we define $g$. 

If we have a pdf of $f_{\text{target}}(z)$ and a cdf of $F_{\text{target}}(z)$, $g$ is simply:

$$g(x) = F_{\text{target}}^{-1}(x).$$

If $f_{\text{target}}(z) > 0 \forall z \in \mathbb R$, then $F_{\text{target}}(z)$ is **monotonically increasing**, meaning that each $F_{\text{target}}$ maps to a unique $x$ value, meaning that $F_{\text{target}}^{-1}(x)$ is well defined. If it were not monotonically increasing, (i.e., two different $x$ values cause the same $F$; $F_{\text{target}}(x_1)=F_{\text{target}}(x_2)$ where $x_1 \ne x_2$), then this would not be the case.


If $f_{\text{target}}(z) \ge 0 \forall z \in \mathbb R$ (opposed to $> 0$), $F_{\text{target}}^{-1}(x)$ is not so well defined. Here, you need to change:

$$F_{\text{target}}^{-1}(x) = \min \left\{ u : F_{\text{target}}(u) \ge x \right\}$$

You can also use this to map $X \sim \text{Uniform}([0,1])$ to a pmf. The cdf form a pmf is a continuous function (compared to discrete points in the pmf function), so the rules stay the same, it's just a piecewise function. 

## PDFs from functions of Random Variables

If $Y = g(X)$, and we want to find $f_Y$ given $f_X$ and $g$ ...

*if g is monotonic* (increasing or decreasing), then it it simply:
...
$f_X(x) =1 \in [0,1]$, and is zero elsewhere. We are told $Y = \ln (X)$. Find the pdf for $Y$. 

$g(x)=\ln(x)$ is monotonically increasing, so we can use this formula. 

$$h(y) = g^{-1}(x)= e^y$$

$$\left| \frac{d (e^y)}{dy} \right|= e^y$$

Therefore,

$$f_Y(y) = f_X(h(y)) \left| \frac{dh(y)}{dy} \right| = 1\cdot e^y = e^y$$


### Non-Invertible Functions

When $g(x)$ is not invertible, you can still solve it, just less easily. This is best illustrated by the example where $g(x)=x^2$. 

$$F_Y=P(Y\le y)$$

by the definition of the CDF. Substitute in $Y= X^2$. 

$$F_Y= P(X^2 \le y) = P(- \sqrt y \le X \le \sqrt y)$$

this is the important bit, where we break down $X$ to its core and see the interval it lays on. Then, again by definition of a CDF:

$$F_Y = F_X(\sqrt y) - F_X(- \sqrt y)$$

Then, to find the PMF for $Y$, you differentiate. 

## Summing Independent Random Variables

If we have something like $Z=X+Y$ and we want to find $f_Z$ given $f_X$ and $f_Y$, we need to apply conditional probability rules.

$$F_{Z|X}(z|x) = P(Z\le z\;|\; X=x) = P(X+Y\le z\;|\; X=x)$$

Because we've pinned $X$ to $x$,

$$F_{Z|X}(z|x)= P(x+Y \le z) = P(Y \le z-x) = F_Y(z-x)$$

Differentiating:

$$f_{Z|X}(z|x) = \frac{dF_{Z|X}}{dz}= \frac{dF_{Y}(z-x)}{dz} = f_Y(z-x)$$

> Not sure I'm following the math here... why is $F_Y \to f_Y$ when we take the derivative w.r.t. $z$. Would this be the case if it was w.r.t $y$ too?
> 
> Ok, $F_Y$ is just a scalar function, so this is just the chain rule.


Then, you can integrate to solve for $f_Z$:

$$
\begin{aligned}
f_Z &= \int_{- \infty}^{\infty} f_{X,Z}(x,z)\,dx\\
...
> *The sum of a large number of independent random variables (with finite variance) has a normal distribution, regardless of what the distributions are.*

This is why normal distributions show up in physics so often; there are many non-gaussian noises/errors that sum together to become gaussian.

Mathematically:

Let $X_1, X_2, \dots, X_N$ be independent and identically distributed (i.i.d) random variables with finite variance. Then,

$$\lim_{N \to \infty} \sum ^N_{i=1}X_i \to \text{Normal}(\cdot)$$

this can be seen here for uniform random variables:

![[Figure_1.png]]


Furthermore, if $X_i$ are *normal* distributions with $\mathbf E[X_i]=\mu$ and $\mathbf E[(X_i-\mu)^2]=\sigma^2$, then:

$$\sum_{i=1}^NX_i \approx \text{Normal}(N\mu, N \sigma^2)$$
