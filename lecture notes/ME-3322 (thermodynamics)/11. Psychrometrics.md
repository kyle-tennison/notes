...
Because the water vapor in the sky isn't that superheated, we can approximate its enthalpy from its saturated vapor:

$$h_v \approx h_g(T) \tag{Moist Air Water Vapor}$$

We can also approximate (I think this is inexact) the entropy of water vapor with:

$$s_v(T, p_v) \approx s_g(T) - R ln(\frac{p_v}{p_g})$$

> Note that you can replace $ln(p_v/{p_g})$ with $ln(\phi)$ 
...
- The dry air and water vapor behave as independent ideal gasses
