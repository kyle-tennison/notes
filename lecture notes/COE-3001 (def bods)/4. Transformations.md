...
Notice that the normal stresses have a **max/min every 90°**, and when $\sigma_{x'x'}$ is maximum, $\sigma_{y'y'}$ is minimum; the same is true vice versa. 

![|313x284](excalidraw-2025-10-23-18.23.25.excalidraw.svg)
%%[🖋 Edit in Excalidraw](excalidraw-2025-10-23-18.23.25.excalidraw.md)%%

Flipping the element 180° means that $\sigma_{xx}$ will be the same (still in tension). But flipping it 90° (or any multiple of 90°) means $\sigma_{xx}$ will be at its minimum (in this case, zero). 

**Whenever normal stresses are at their maxima or minima, shear stress zero. Here, the normal stresses are the *principal stresses.*** 
...
- The principal stresses are $\sigma_1$ and $\sigma_2$, where $\sigma_1$ is at its max and $\sigma_2$ is at its min. Hence, it is always true that $\sigma_1 > \sigma_2$. 

In 3D, there is a third principal stress $\sigma_3$. 
...
## Eigenvalue Analysis for Principal Stress
...
For a **3D problem**, there are three principal stresses $\sigma_1, \sigma_2, \sigma_3$. The maximum shear stress that *may likely not lay within the same plane* is:
