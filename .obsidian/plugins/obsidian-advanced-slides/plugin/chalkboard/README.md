...
With this plugin you can add a chalkboard to reveal.js. The plugin provides two possibilities to include handwritten notes in your presentation:
...
The plugin records all drawings made so that they can be played back using the `autoSlide` feature or the `audio-slideshow` plugin.
...
```
has to be included in the `head` section of your HTML file.
...
- Click on the up/down arrows on the left to switch among multiple chalkboards (the up/down arrows are only available for the chalkboard)
- Click the left mouse button and drag to write on the notes canvas or chalkboard
- Click the right mouse button and drag to wipe away previous drawings
- Touch and move to write on the notes canvas or chalkboard
...
The following configuration options allow you to change the appearance of the notes canvas and the chalkboard. All of these options require two values, the first gives the value for the notes canvas, the second for the chalkboard.
...
Reveal.initialize({
    ...
    chalkboard: {
        ...
        boardmarkers : [
                ...
                { color: 'rgba(150,0,201,1)', cursor: 'url(' + path + 'img/boardmarker-purple.png), auto'},
